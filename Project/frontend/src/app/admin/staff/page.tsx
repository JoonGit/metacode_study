...
export default function AdminStaff() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await backendApi.get('/admin/staff');
  ...
  const handleSaveUser = async () => {
    try {
      ...
      if (editingUser) {
        await backendApi.put(`/admin/staff/${editingUser.userId}`, payload);
        ...
        await backendApi.post('/admin/staff', payload);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">관리자 및 직원 계정 관리</h2>
        <button 
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <option value="ALL">전체 보기</option>
            <option value={UserType.ADMIN}>시스템관리자</option>
          </select>
        </div>
      </div>
